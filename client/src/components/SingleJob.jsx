...
function SingleJob({
  ...
  return (
    <article className='bg-[#f2f2f2] p-4 lg:p-6 shadow-md rounded'>
      ...
      <div className='mt-4 lg:mt-6 grid lg:grid-cols-2 gap-y-2'>
        <p className='flex items-center text-base lg:text-lg gap-x-3 capitalize'>
          <span>
            <FaBriefcase />
          </span>
          {jobType}
      ...
      <footer className='flex gap-x-2 mt-3 lg:mt-4'>
        <Link to={`/dashboard/edit-job/${_id}`} className='btn capitalize'>
          edit
        </Link>
        <Form method='post' action={`/dashboard/delete-job/${_id}`}>
          <button type='submit' className='delete-btn capitalize'>
            Delete
          </button>
        </Form>
      </footer>
